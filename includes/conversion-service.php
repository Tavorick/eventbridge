...
class EventBridge_Conversion_Service {
	...
	public function ensure_open_from_link( $profile_link, $provider, $entity_type, $external_id, $conversion_event_ids = array() ) {
		...
		return $this->repository->ensure_open( $profile_link, $provider, $entity_type, $external_id, $conversion_event_ids, $this->build_attribution_snapshot( $profile_id ) );
	}
	...
	private function build_attribution_snapshot( $profile_id ) {
	...
	private function is_safe_occurrence( $occurrence ) {
		...
		if ( 'other' === $action_source && ! empty( $occurrence['event_source_url'] ) ) return false;
